...
- Before each reply, whale-chan's avatar appears in the transcript — a 200×200 px
  portrait in terminals with inline-image support (Kitty, iTerm2). Terminals
  without image support show a compact text badge instead. The avatar is
  display-only: it never enters the model context or your token usage.
